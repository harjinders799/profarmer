...
export default function List({ data }) {
    ...
    if (data.length) {
        ...
        Object.keys(grp).map(v => arr.push({ giver: v, total: sumBy(grp[v], o => parseInt(o.amount)), data: grp[v] }))
    }
                    ...
                    <View style={styles.list}>
                        <TouchableOpacity onPress={() => navigate('Detail', { item })}>
                            <View style={styles.row}>
                                <Text numberOfLines={1} h3 style={{ width: '80%' }}>{item?.giver}</Text>
                                <Text h5 >{strings.view}</Text>
                                {/* <Icon name="more-horizontal" type="Feather" size={20} /> */}
                            </View>
                            <View style={styles.row}>
                                <Text numberOfLines={1} h4>{strings.total_principal}</Text>
                                <Text numberOfLines={1} h4>{currencyFormat(item?.total)}</Text>
                            </View>
                        </TouchableOpacity>
                    </View>
                )

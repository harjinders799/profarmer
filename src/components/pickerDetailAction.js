...
export default function PickerDetailAction({ data, picker }) {
    ...
    const delteData = async () => {
        Alert.alert(strings.weight, `${strings.delete_wt} ${data?.weight}Kg`,
            [
                {
                    text: 'Yes',
                    onPress: async () => {
    ...
    return (
        <View style={styles.list}>
            ...
            <View style={styles.row}>
                <Text h3 numberOfLines={1} style={styles.picker}>{picker ? dateFormat(data?.date) : data?.picker}</Text>
                <Text h3 numberOfLines={1} style={styles.wt}>{data?.weight}kg</Text>
            </View>

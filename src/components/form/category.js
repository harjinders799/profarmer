...
export default function Category(props) {
    ...
    const { selectedCat, setSelectedCat, category } = props;
    ...
    const onAdd = async () => {
        if (catName.trim() != "" && catName) {
            setLoading(true);
            let cat = categories;
            var foundId = findIndex(categories, (x) => { return x == catName });
            if (foundId > -1) {
    ...
    return (
        <View style={styles.screen}>
            ...
            <TouchableOpacity disabled={category?true:false} style={[styles.cat]} onPress={() => setModalVisible(true)}>
                {selectedCat || category ?
                    <Text h4 medium >{category ?? selectedCat}</Text>
                    :
                    <Input
                        editable={false}
                ...
                <Icon type="FontAwesome" name="chevron-down" size={14} color={colors.secondaryText} />
            </TouchableOpacity>
            <Modal
                ...
                <View style={styles.modal}>
                    ...
                    {addCat ?
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) =>
                                <TouchableOpacity style={styles.list} onPress={() => { setSelectedCat(item); setModalVisible(false); }}>
                                    <Text h3>{item}</Text>
                                    {selectedCat == item && <Icon type="MaterialIcons" name="check" size={25} color={colors.primary} />}
                                </TouchableOpacity>
                            }

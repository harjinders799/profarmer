...
export default function DashBoard({ navigation }) {
    ...
    if (data.length) {
        ...
        Object.keys(grp).map(v => arr.push({ date: v, total: sumBy(grp[v], o => parseInt(o.amount)), data: grp[v] }))

    }
    useEffect(() => {
        if (Auth()?.currentUser?.uid && Array.isArray(givers) && givers.length < 1 && Array.isArray(data) && data.length) {
            let pick = [];
            data.map(v => {

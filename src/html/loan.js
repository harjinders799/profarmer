    ...
    <html>
      ...
      <body>
        <div style="display: flex; flex-direction:column; align-items:center">
          ...
          <h2>${data?.name}</h2>
          <div>
            <h3>${strings.interest}: ${currencyFormat(data?.interest_rate)} </h3>
          </div>
        </div>
        <div style="display: flex; gap: 16px; justify-content: space-between;">
          <div class="card" style="background-color:#d63122;">
            <h1>${strings.taken_amount}</h1>
            <h3>${strings.taken_amount}: ${currencyFormat(data?.totalReceivedAmount, 2)} </h3>
            <h3>${strings.interest}: ${currencyFormat(data?.totalReceivedAmountInterest, 2)} </h3>
            <h3>${strings.taken_amount_with_interest}: ${currencyFormat(data?.totalReceivedAmountWithInterest, 2)}
            </h3>
          </div>
          <div class="card" style="background-color:#4CA15f;">
             <h1>${strings.given_amount}</h1>
            <h3>${strings.given_amount}: ${currencyFormat(data?.totalGivenAmount, 2)} </h3>
            <h3>${strings.interest}: ${currencyFormat(data?.totalGivenAmountInterest, 2)} </h3>
            <h3>${strings.given_amount_with_interest}: ${currencyFormat(data?.totalGivenAmountWithInterest, 2)}
          </div>
        </div>
        <div style="display: flex; justify-content: space-around; margin: 0 10%; color: #FFA500;">
            <h2>${strings.final}</h3>
            <h5> ${data?.finalAmount == 0 ? '---' : data?.finalAmount > 0 ? strings.receive : strings.give}</h6>
    ...
    record => {
      ...
      return `<tr>
                ...
                <td>${currencyFormat(interest)}</td>
                 <td>${record?.type == 'giver' ? currencyFormat(record?.amount) : '--'}</td>
                 <td>${record?.type == 'receiver' ? currencyFormat(record?.amount) : '--'}</td>
                <td>${currencyFormat(
